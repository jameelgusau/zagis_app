...
export function DeleteDepartment({ open, setOpen, selected }: { open: boolean, setOpen: Dispatch<SetStateAction<boolean>>, selected: Department}) {

  const { setDepartments } = useDashboardStore((s) => s);
  const { deleteDepartment: { path } } = APIS
    ...
    <Dialog open={open} onOpenChange={() => setOpen((prev) => !prev)}>
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)} >
          <DialogHeader>
            <DialogTitle className="text-bold">Delete Department</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col justify-center m-3 items-center">
            ...
            <p className="text-slate-500">Do you really want to remove <span className="font-bold">{selected.department_name}</span> ?</p>
