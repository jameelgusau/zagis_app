...
export function DeletePurpose({ open, setOpen, selected }: { open: boolean, setOpen: Dispatch<SetStateAction<boolean>>, selected: PurposeType }) {
  const { setPurposes } = useDashboardStore((s) => s);
  const { deletePurpose: { path } } = APIS
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)} >
          ...
          <div className="flex flex-col justify-center m-3 items-center">
            ...
            <p className="text-slate-500">Do you really want to remove <span className="font-bold">{selected.purpose_name}</span> ? </p>
