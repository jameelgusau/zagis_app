...
export function DeleteRank({ open, setOpen, selected }: { open: boolean, setOpen: Dispatch<SetStateAction<boolean>>, selected: Rank }) {
  const { setRanks } = useDashboardStore((s) => s);
  const { deleteRank: { path } } = APIS
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)} >
          ...
          <div className="flex flex-col justify-center m-3 items-center">
            ...
            <p className="text-slate-500">Do you really want to remove <span className="font-bold">{selected.rank_name}</span> ? </p>
